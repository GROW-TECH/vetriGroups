...
// Update role labels to include site-engineer
...
const ALL_ROLES: (EmployeeRole | 'all')[] = ['all', 'labor', 'mason', 'engineer', 'site-engineer', 'supervisor'];

// Helper function to format Aadhaar number with spaces
...
export default function EmployeeListScreen() {
  ...
  const canEdit = user?.role === 'admin' || user?.role === 'engineer' || user?.role === 'site-engineer';
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(e => {
      ...
      const matchesSearch = e.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        (e.aadhaarNo && e.aadhaarNo.includes(searchQuery.replace(/\s/g, '')));
  ...
  const handleShare = async () => {
    ...
    // Add Aadhaar if available
    if (menuEmployee.aadhaarNo) {
      message += `Aadhaar: ${formatAadhaar(menuEmployee.aadhaarNo)}\n`;
    }
    
    message += `Daily Salary: ₹${(menuEmployee.salary || 0).toLocaleString()}`;
  ...
  const renderEmployee = ({ item }: { item: Employee }) => {
    ...
    const employeeAadhaar = item?.aadhaarNo;
        ...
        <View style={styles.employeeInfo}>
          ...
          {employeeAadhaar && (
            <View style={styles.aadhaarRow}>
              <Feather name="credit-card" size={12} color={Colors.light.primary} />
              <ThemedText type="small" style={{ color: Colors.light.primary, marginLeft: Spacing.xs, fontWeight: '500' }}>
                {formatAadhaar(employeeAadhaar)}
              </ThemedText>
            </View>
          )}
          ...
          <View style={[styles.menuContent, { backgroundColor: theme.backgroundDefault }]}>
            <View style={styles.menuHeader}>
              ...
              <View style={{ marginLeft: Spacing.md, flex: 1 }}>
                ...
                {menuEmployee?.aadhaarNo && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                    <Feather name="credit-card" size={12} color={Colors.light.primary} />
                    <ThemedText type="small" style={{ color: Colors.light.primary, marginLeft: 4, fontWeight: '500' }}>
                      {formatAadhaar(menuEmployee.aadhaarNo)}
                    </ThemedText>
                  </View>
                )}
              </View>
              ...
              <View style={styles.editForm}>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <View style={styles.roleOptions}>
                    {['labor', 'mason', 'engineer', 'site-engineer', 'supervisor'].map((role) => (
                      <Pressable
                        key={role}
