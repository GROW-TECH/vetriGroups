...
// Add these functions to your DashboardScreen component



const handleHideAppointment = async (appointmentId: string) => {
  try {
    const appointmentRef = doc(db, 'appointments', appointmentId);
    await updateDoc(appointmentRef, {
      status: 'read', // Mark as read instead of hiding
      updatedAt: new Date(),
    });
...
const handleHideVendorOrder = async (orderId: string) => {
  try {
    const orderRef = doc(db, 'materialOrders', orderId);
    await updateDoc(orderRef, {
      vendorRead: true, // Mark as read by vendor
      updatedAt: new Date(),
    });
    // Optionally hide locally
    setHiddenRequests(prev => new Set([...prev, orderId]));
  } catch (error) {
    console.error('Error hiding vendor order:', error);
  }
};
  return (
    <Animated.View
  ...
  if (user?.role === 'admin') {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        // Skip locally hidden appointments
        if (hiddenAppointments.has(docSnap.id)) {
          continue;
        }
...
}, [user, hiddenAppointments]); // Add hiddenAppointments dependency
...
useEffect(() => {
  ...
  const q = query(
    ...
    where('supplierId', '==', String(user.vendorId)),
    where('vendorRead', '==', false) // Only show unread orders
  );
...
useEffect(() => {
  ...
  const q = query(
    ...
    where('type', '==', 'payment'),
    where('status', '!=', 'read') // Add this line to exclude read items
  );
  ...
  const unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      const data = docSnap.data();
      // Also filter locally hidden items
      if (!hiddenRequests.has(docSnap.id)) {
        list.push({
          id: docSnap.id,
  ...
  return () => unsubscribe();
}, [user, hiddenRequests]); // Add hiddenRequests dependency
useEffect(() => {
  if (user?.role !== 'admin') return;
  ...
  // Add after handleCardPress function
const handleHidePaymentRequest = async (requestId: string) => {
  try {
    ...
    console.error('Error hiding payment request:', error);
  }
};
      ...
      <ThemedText type="body" style={{ fontWeight: '700' }}>
        Payment Notifications
            ...
            {visiblePaymentRequests.map((req) => {
              const ui = getPaymentStatusUI(req.status || 'pending');


  return (
    <Swipeable
      key={req.id}
onSwipeableOpen={() => handleHidePaymentRequest(req.id)}      renderRightActions={() => (
        <View style={{ 
...
{order.vendorStatus === 'pending' ? (
  ...
  >
    ...
    >
      {(order.vendorStatus || 'pending').toUpperCase()}
  ...
  <Swipeable
    key={appointment.id}
onSwipeableOpen={() => handleHideAppointment(appointment.id)}
    renderRightActions={() => (
      <View style={{ 
        backgroundColor: '#10B981', 
        justifyContent: 'center', 
