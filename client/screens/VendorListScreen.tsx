...
const filteredVendors = React.useMemo(() => {
  // If logged in user is vendor → show only their vendor
  if (user?.role === 'vendor') {
    return vendors.filter(
      v => v.id === user.vendorId
    );
  }

  // Admin / Engineer → show all vendors
  return vendors;
}, [vendors, user]);
...
if (!vendorName.trim() || !username.trim() || !password.trim() || !phone.trim()) {
  Alert.alert('Missing Fields', 'Please enter vendor name, username, password and phone number.');
  return;
}
