...
export default function VendorDetailScreen() {
  ...
  // Define getRandomColor first (before using it)
  const getRandomColor = () => {
    const colors = ['#6366F1', '#F59E0B', '#EF4444', '#06B6D4', '#10B981', '#EC4899', '#8B5CF6', '#F97316'];
    return colors[Math.floor(Math.random() * colors.length)];
  };

  // Get all unique categories from existing vendor materials
  const existingCategories = [...new Set(vendor?.materials.map(m => m.category) || [])];
  
  // Create custom categories from existing materials that aren't in MATERIAL_CATEGORIES
  const existingCustomCategories = existingCategories
    .filter(cat => !MATERIAL_CATEGORIES.find(mc => mc.id === cat))
    .map(cat => ({
      id: cat,
  ...
  const allCategories = [...customCategories, ...existingCustomCategories, ...MATERIAL_CATEGORIES];
  ...
  const getCategoryLabel = (category: string) => {
    ...
    const existingCat = existingCustomCategories.find(c => c.id === category);
  ...
  const getCategoryColor = (category: string) => {
    ...
    // Check existing custom categories
    const existingCat = existingCustomCategories.find(c => c.id === category);
    if (existingCat) return existingCat.color;
