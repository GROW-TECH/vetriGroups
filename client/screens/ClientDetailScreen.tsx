...
import { ProjectFile, PaymentStage, Appointment, Client, Transaction } from '@/types';
...
export default function ClientDetailScreen() {
  ...
  const [editedProjectName, setEditedProjectName] = useState(client?.projectName || '');
  const [editedClientName, setEditedClientName] = useState(client?.name || '');
  const [editedOwnerPhone, setEditedOwnerPhone] = useState(client?.ownerPhone || '');
...
const clientPhotos = useMemo(() => {
  ...
  const clientStages = useMemo(() => {
    return paymentStages.filter(s => s.clientId === clientId);
  }, [paymentStages, clientId]);
 ...
 const tabs = [
  ...
  { key: 'photos', label: 'Photos', icon: 'image' }, // ✅ NEW
  { key: 'payments', label: 'Payments', icon: 'credit-card' },
  { key: 'appointments', label: 'Schedule', icon: 'calendar' },
];


const handleUploadFile = async (type: 'plan' | 'agreement' | 'photo') => {
    try {
  ...
  const handleRazorpaySuccess = async (paymentId: string, orderId: string) => {
    ...
    const updated = { ...selectedStageForPayment, isPaid: true };
    await updatePaymentStage(updated);
  ...
  const handlePaymentOptionSelect = async (option: PaymentOption) => {
    if (!selectedStageForPayment) return;

    Alert.alert(
      'Payment Confirmation',
      `Pay ${selectedStageForPayment.amount.toLocaleString()} via ${option.name}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Confirm',
  ...
  const handleSaveEdits = async () => {
    ...
    // Save edited client details as well
    if (client) {
      const updatedClient: Client = {
        ...client,
        projectName: editedProjectName,
        name: editedClientName,
        ownerPhone: editedOwnerPhone || undefined,
      };
      // Client update functionality removed - updateClient not available in DataContext
    }
  ...
  const paidStagesCount = clientStages.filter(s => s.isPaid).length;
  const pendingStagesCount = clientStages.filter(s => !s.isPaid).length;
  const nextPendingStage = clientStages.find(s => !s.isPaid);
              ...
              <>
                ...
                {client?.username && client?.password ? (
                  <View style={{ marginTop: Spacing.md, backgroundColor: theme.backgroundSecondary, borderRadius: 10, padding: Spacing.md }}>
                    <ThemedText type="small" style={{ color: theme.textSecondary, fontWeight: '600' }}>Login Credentials</ThemedText>
                    <ThemedText type="body" style={{ color: theme.text, marginTop: 2 }}>Username: <ThemedText style={{ fontWeight: 'bold' }}>{client.username}</ThemedText></ThemedText>
                    <ThemedText type="body" style={{ color: theme.text, marginTop: 2 }}>Password: <ThemedText style={{ fontWeight: 'bold' }}>{client.password}</ThemedText></ThemedText>
                  </View>
                ) : null}
              </>
            )}
      ...
      <View style={styles.tabContent}>
        <View style={styles.fileSection}>
          <View style={styles.fileSectionHeader}>
            ...
            {canEdit ? (
              <Pressable onPress={() => handleUploadFile('plan')} style={styles.uploadButton}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.fileScroll}>
              {plans.map(file => (
                ...
                >
                  ...
                  {canEdit ? (
                    <Pressable onPress={() => handleDeleteFile(file.id)} style={styles.deleteButton}>
          ...
          return (
            <View key={stage.id} style={[styles.paymentStageCard, { backgroundColor: theme.backgroundDefault }]}>
              ...
              <View style={styles.paymentStageContent}>
                ...
                {!stage.isPaid && !isEditMode && canPay ? (
                  <Pressable onPress={() => handlePayNow(stage)} style={styles.payNowButton}>
                    <Feather name="credit-card" size={16} color="#FFFFFF" />
                    <ThemedText type="small" style={{ color: '#FFFFFF', fontWeight: '600', marginLeft: Spacing.xs }}>Pay Now</ThemedText>
                  </Pressable>
                ) : null}
              </View>
            </View>
      ...
      {clientAppointments.length > 0 ? (
        clientAppointments.map(apt => (
          <View key={apt.id} style={[styles.appointmentCard, { backgroundColor: theme.backgroundDefault }]}>
            ...
            <View style={[
              styles.statusPill,
              { backgroundColor: apt.status === 'approved' ? Colors.light.success + '15' : apt.status === 'rejected' ? Colors.light.error + '15' : Colors.light.warning + '15' }
            ]}>
              <ThemedText
                type="small"
                style={{
                  color: apt.status === 'approved' ? Colors.light.success : apt.status === 'rejected' ? Colors.light.error : Colors.light.warning,
...
logoutButton: {
  ...
  justifyContent: 'center',
  borderRadius: BorderRadius.sm,
  marginLeft: Spacing.sm,  // ADD THIS LINE
},
header: {
    padding: Spacing.lg,
  },
