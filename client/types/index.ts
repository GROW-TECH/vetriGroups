...
export interface Employee {
  ...
  age: number;
  address: string;
  aadhaarNo?: string; // 🆕 Added: 12-digit Aadhaar number (stored without spaces)
  role: EmployeeRole;
  salary: number;
  faceImage?: string;
  ...
  // 🆕 Added: Login credentials for engineer and site-engineer roles
  username?: string;
  password?: string;
...
export interface ProjectFile {
  ...
  title: string;                 // ✅ ADD (user-entered title)
  name: string;                  // original file name
  type: 'plan' | 'agreement' | 'photo';

  uri: string;                   // local / firebase url
  mimeType: string;              // ✅ ADD (pdf, image, doc, etc.)

  uploadedByRole: UserRole;      // ✅ ADD (admin / engineer)
  uploadedByName: string;        // ✅ ADD
